        ...
        const SURAH_INDONESIA = ["Pembukaan","Sapi Betina","Keluarga 'Imran","Wanita","Jamuan","Binatang Ternak","Tempat yang Tertinggi","Harta Rampasan Perang","Pengampunan","Nabi Yunus","Nabi Hud","Nabi Yusuf","Guruh","Nabi Ibrahim","Gunung Al-Hijr","Lebah","Perjalanan Malam","Gua","Maryam","Ta Ha","Nabi-Nabi","Haji","Orang-Orang Mukmin","Cahaya","Pembeda","Penyair","Semut","Kisah-Kisah","Laba-Laba","Bangsa Romawi","Luqman","Sajdah","Golongan yang Bersekutu","Kaum Saba'","Pencipta","Ya Sin","Barisan-Barisan","Sad","Rombongan-Rombongan","Maha Pengampun","Yang Dijelaskan","Musyawarah","Perhiasan","Kabut","Yang Berlutut","Bukit-Bukit Pasir","Nabi Muhammad","Kemenangan","Kamar-Kamar","Qaf","Angin yang Menerbangkan","Bukit Tursina","Bintang","Bulan","Maha Pengasih","Hari Kiamat","Besi","Gugatan","Pengusiran","Wanita yang Diuji","Barisan","Hari Jumat","Orang-Orang Munafik","Hari Dinampakkan Kesalahan","Talak","Pengharaman","Kerajaan","Pena","Hari Kiamat","Tempat Naik","Nabi Nuh","Jin","Orang yang Berselimut","Orang yang Berkemul","Hari Kiamat","Manusia","Malaikat yang Diutus","Berita Besar","Malaikat yang Mencabut","Ia Bermuka Masam","Menggulung","Terbelah","Orang-Orang yang Curang","Terbelah","Gugusan Bintang","Yang Datang di Malam Hari","Yang Paling Tinggi","Hari Kiamat","Fajar","Negeri","Matahari","Malam","Waktu Duha","Lapang","Buah Tin","Segumpal Darah","Kemuliaan","Pembuktian","Kegoncangan","Kuda Perang","Hari Kiamat","Bermegah-Megahan","Masa","Pengumpat","Gajah","Orang Quraisy","Barang yang Berguna","Nikmat yang Banyak","Orang-Orang Kafir","Pertolongan","Gejolak Api","Ikhlas","Waktu Subuh","Manusia"];
        ...
        const colListTitle = document.getElementById('col-list-title');
        const colListHeaders = document.getElementById('col-list-headers');

        window.switchNav = function(btn, type) {
            document.querySelectorAll('.nav-menu .nav-item').forEach(el => el.classList.remove('active'));
            if(btn) btn.classList.add('active');
            
            if(type === 'quran' || type === 'home') {
                colListTitle.innerText = "Al-Qur'an";
                colListHeaders.style.display = 'flex';
                renderQuranList();
            } else if (type === 'hadist') {
                colListTitle.innerText = "Hadist";
                colListHeaders.style.display = 'none';
                renderHadithLeftList();
            } else if (type === 'komunitas') {
                colListTitle.innerText = "Komunitas";
                colListHeaders.style.display = 'none';
                renderKomunitasMain();
            } else if (type === 'tafsir') {
                colListTitle.innerText = "Tafsir Lengkap";
                colListHeaders.style.display = 'none';
                surahContainer.innerHTML = '<div style="padding:30px 20px; color:var(--text-muted); text-align:center; line-height:1.6; font-size:1.05em;">Fitur <b>Tafsir Terpadu</b> terintegrasi langsung dengan sangat cerdas di dalam layar baca Al-Qur\'an Anda.<br><br>Cukup klik tombol <b style="color:var(--accent-gold);">📖 Tafsir</b> tepat di bawah ayat yang sedang Anda baca, dan seluruh tafsir (Kemenag, Ibnu Katsir, Jalalain) akan terbuka instan tanpa perlu berpindah layar!</div>';
            }
        };
        ...
        function renderQuranList() {
            ...
            ALL_SURAHS.forEach((surahName, index) => {
                const num = index + 1;
                const idName = SURAH_INDONESIA[index];
                const div = document.createElement('div');
                div.className = `s-item`;
                div.innerHTML = `<span class="col-num">${num}.</span><span class="col-ar">${surahName}</span><span class="col-en" style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="${idName}">${idName}</span>`;
                
                div.addEventListener('click', async function() {
                document.querySelectorAll('.s-item').forEach(s => s.classList.remove('active'));
                this.classList.add('active');

                // Tampilkan play button, sembunyikan pencarian hadist
                if(document.getElementById('btn-play')) document.getElementById('btn-play').style.display = 'flex';
                if(document.getElementById('hadith-search-container')) document.getElementById('hadith-search-container').style.display = 'none';
                ...
                try {
                    ...
                    dataSurah.ayat.forEach(ayat => {
                        ...
                        htmlAyat += `
                        <div class="verse-group" style="cursor:pointer; transition:0.3s; padding:10px; border-radius:12px;" onclick="showTafsirs(this, ${num}, ${nomorAyat}, '${dataSurah.namaLatin.replace(/'/g, "\\'")}')" onmouseover="this.style.background='rgba(56, 189, 248, 0.05)'" onmouseout="this.style.background='transparent'">
                            ...
                            <div class="tafsir-action" style="margin-top:10px;">
                                ...
                                <button class="btn-outline" style="border-color:var(--accent-gold); color:var(--accent-gold); padding:5px 15px; font-size:0.8em;" onclick="showTafsirs(this, ${num}, ${nomorAyat}, '${dataSurah.namaLatin.replace(/'/g, "\\'")}')">📖 Tafsir & Hadist</button>
        ...
        function renderHadithLeftList() {
            surahContainer.innerHTML = '<div style="padding:20px; color:var(--accent-emerald); text-align:center;">Memuat kitab hadist...</div>';
            const drawList = () => {
                surahContainer.innerHTML = '';
                window.HADITH_LIST.forEach((h, index) => {
                    const div = document.createElement('div');
                    div.className = `s-item`;
                    ...
                    div.innerHTML = `
                        <div style="font-weight:bold; color:var(--text-main); font-size:1.05em; display:flex; align-items:center; gap:8px;">
                            <span style="font-size:1.2em;">📖</span> ${h.name}
                        </div>
                        <div style="font-size:0.75em; font-weight:bold; color:#fff; background:var(--accent-emerald); padding:3px 10px; border-radius:12px; box-shadow:0 2px 5px rgba(16,185,129,0.3);">
                    ...
                    div.addEventListener('click', () => {
                        ...
                        // Render directly to Main Column (Column 3)
                        window.executeLoadHadithBookMain(h.id, h.name.replace(/'/g, "\\'"), h.total);
                    });
                    surahContainer.appendChild(div);
                });
            };

            if (window.HADITH_LIST) {
                drawList();
            } else {
                const listScript = document.createElement('script');
                listScript.src = 'QuranHadist/data/hadith_all_js/hadith_list.js';
                listScript.onload = drawList;
        ...
        // OLD TAFSIR LOGIC REMOVED TO PREVENT OVERWRITING NEW INLINE POPUP LOGIC
        ...
        window.filterTafsirLibrary = function() {
            ...
            document.getElementById('tafsirLibraryBody').querySelectorAll('.tafsir-card').forEach(card => {
        ...
        // --- RESTORED: HADITH LIBRARY LOGIC ---
        window.CURRENT_HADITH_BOOK = null;
        window.CURRENT_HADITH_TOTAL = 0;
        
        window.openHadithLibrary = function() {
            document.getElementById('hadithLibraryModal').classList.add('active');
            backToHadithBooks();
            
            const libBody = document.getElementById('hadithLibraryBody');
            if (libBody.innerHTML.trim() === '' || libBody.innerHTML.includes('Diisi oleh JS')) {
                if(!window.HADITH_LIST) {
                    libBody.innerHTML = "<p style='color:var(--accent-emerald); padding:20px;'>Memuat 9 Kitab Hadist... ⏳</p>";
                    const listScript = document.createElement('script');
                    listScript.src = 'QuranHadist/data/hadith_all_js/hadith_list.js';
                    listScript.onload = () => renderHadithList(window.HADITH_LIST);
                    document.body.appendChild(listScript);
                } else {
                    renderHadithList(window.HADITH_LIST);
                }
            }
        };

        function renderHadithList(list) {
            const libBody = document.getElementById('hadithLibraryBody');
            libBody.style.display = 'grid';
            libBody.style.gridTemplateColumns = 'repeat(auto-fill, minmax(250px, 1fr))';
            libBody.style.gap = '15px';
            libBody.innerHTML = list.map(h => `
                <div class="hadith-book-card" data-name="${h.name.toLowerCase()}" style="border:1px solid rgba(16,185,129,0.3); border-radius:12px; padding:20px; cursor:pointer; background:linear-gradient(145deg, rgba(16,185,129,0.05) 0%, rgba(0,0,0,0.5) 100%); transition:all 0.3s;" onclick="executeLoadHadithBook('${h.id}', '${h.name.replace(/'/g, "\\'")}', ${h.total})" onmouseover="this.style.transform='translateY(-5px)'; this.style.borderColor='var(--accent-emerald)'; this.style.boxShadow='0 10px 20px rgba(16,185,129,0.2)'" onmouseout="this.style.transform='translateY(0)'; this.style.borderColor='rgba(16,185,129,0.3)'; this.style.boxShadow='none'">
                    <h3 style="color:var(--accent-emerald); margin:0 0 10px 0; font-size:1.3em;">${h.name}</h3>
                    <p style="color:var(--text-muted); margin:0; font-size:0.9em;">Total: ${h.total.toLocaleString()} Hadist</p>
                    <div style="margin-top:15px; font-size:0.8em; color:var(--accent-emerald); opacity:0.8;">Klik untuk membuka →</div>
                </div>
            `).join('');
        }

        window.filterHadithLibrary = function() {
            const q = document.getElementById('searchHadithBook').value.toLowerCase();
            document.getElementById('hadithLibraryBody').querySelectorAll('.hadith-book-card').forEach(card => {
        ...
        window.executeLoadHadithBookMain = function(bookId, bookName, total) {
            window.CURRENT_HADITH_BOOK = bookId;
            window.CURRENT_HADITH_TOTAL = total;
            
            document.getElementById('main-surah-title').innerText = `Kitab ${bookName}`;
            document.getElementById('main-surah-sub').innerText = `${total.toLocaleString()} Hadist Tersedia`;
            
            if(document.getElementById('btn-play')) document.getElementById('btn-play').style.display = 'none';
            if(document.getElementById('hadith-search-container')) document.getElementById('hadith-search-container').style.display = 'flex';
            
            document.getElementById('mainHadithNumberInput').placeholder = total;
            document.getElementById('mainHadithNumberInput').max = total;
            
            const readingBody = document.getElementById('reading-body');
            readingBody.innerHTML = `
                <div id="main-hadith-content">
                    <div style="text-align:center; padding:80px 20px; color:var(--text-muted);">
                        <div style="font-size:4em; margin-bottom:20px;">📖</div>
                        <h3 style="color:var(--accent-emerald); margin-bottom:10px;">Perpustakaan Hadist Aktif</h3>
                        <p>Ketikkan nomor hadist pada kotak pencarian di pojok kanan atas untuk mulai membaca.</p>
                        <button onclick="jumpToHadithNumberMain(1)" style="background:rgba(16,185,129,0.2); border:1px solid var(--accent-emerald); color:#fff; padding:10px 20px; border-radius:8px; cursor:pointer; margin-top:20px; font-weight:bold; font-size:1.1em;">Atau Baca Hadist No. 1</button>
                    </div>
                </div>
            `;
        };

        window.jumpToHadithNumberMain = function(forceNum) {
            let num = forceNum;
            if(!num) {
                const input = document.getElementById('mainHadithNumberInput').value;
                num = parseInt(input);
            }
            if(isNaN(num) || num < 1 || num > window.CURRENT_HADITH_TOTAL) {
                alert('Nomor hadist tidak valid. Masukkan angka antara 1 sampai ' + window.CURRENT_HADITH_TOTAL);
                return;
            }
            if(document.getElementById('mainHadithNumberInput')) {
                document.getElementById('mainHadithNumberInput').value = num;
            }
            
            const contentBody = document.getElementById('main-hadith-content');
            contentBody.innerHTML = '<div style="color:var(--accent-emerald); text-align:center; padding:40px;">Mencari riwayat hadist... ⏳</div>';
            
            const CHUNK_SIZE = 500;
            const chunkIndex = Math.floor((num - 1) / CHUNK_SIZE);
            const scriptId = 'hadith_chunk_' + window.CURRENT_HADITH_BOOK + '_' + chunkIndex;
            
            const displayHadith = () => {
                const cache = window.HADITH_CACHE[`${window.CURRENT_HADITH_BOOK}_${chunkIndex}`];
                let htmlOut = '';
                let lastRenderedNum = num;
                let countFound = 0;

                for (let i = 0; i < 20; i++) {
                    const currentNum = num + i;
                    if (currentNum > window.CURRENT_HADITH_TOTAL) break;
                    
                    if (Math.floor((currentNum - 1) / CHUNK_SIZE) !== chunkIndex) break;
                    
                    if(cache && cache[currentNum]) {
                        const hData = cache[currentNum];
                        countFound++;
                        lastRenderedNum = currentNum;
                        htmlOut += `
                            <div style="background:rgba(16,185,129,0.05); border:1px solid rgba(16,185,129,0.3); border-radius:12px; padding:30px; margin-bottom:30px; position:relative;">
                                <div style="position:absolute; top:-15px; left:20px; background:var(--accent-emerald); color:#fff; padding:5px 20px; border-radius:20px; font-weight:bold; box-shadow:0 4px 10px rgba(16,185,129,0.3);">
                                    Hadist No. ${currentNum}
                                </div>
                                <div style="font-family:'Amiri',serif; font-size:2.2em; direction:rtl; text-align:right; color:#fff; margin:30px 0 20px 0; line-height:1.8;">${hData.ar}</div>
                                <div class="translation-text" style="font-size:1.1em; color:var(--text-main); line-height:1.8; text-align:justify;">"${hData.id}"</div>
                            </div>
                        `;
                    }
                }
                ...
                if (countFound === 0) {
                    htmlOut = '<div style="color:#ff4d4d; text-align:center; padding:40px;">Hadist nomor ' + num + ' tidak ditemukan atau penomoran melompat. Coba nomor lain.</div>';
                }

                const prevNum = Math.max(1, num - 20);
                const nextNum = lastRenderedNum + 1;
                
                htmlOut += `
                    <div style="margin-top:20px; display:flex; justify-content:space-between; padding-bottom:50px;">
                        ${num > 1 ? `<button onclick="jumpToHadithNumberMain(${prevNum})" style="background:transparent; border:1px solid var(--accent-emerald); color:var(--accent-emerald); padding:8px 15px; border-radius:5px; cursor:pointer;">← Sebelumnya</button>` : '<div></div>'}
                        ${nextNum <= window.CURRENT_HADITH_TOTAL ? `<button onclick="jumpToHadithNumberMain(${nextNum})" style="background:var(--accent-emerald); border:none; color:#fff; padding:8px 15px; border-radius:5px; cursor:pointer;">Selanjutnya →</button>` : '<div></div>'}
                    </div>
                `;

                contentBody.innerHTML = htmlOut;
            ...
            if(!document.getElementById(scriptId)) {
                const script = document.createElement('script');
                script.id = scriptId;
                script.src = `QuranHadist/data/hadith_all_js/${window.CURRENT_HADITH_BOOK}_${chunkIndex}.js`;
                script.onload = displayHadith;
                script.onerror = () => { contentBody.innerHTML = '<div style="color:red; text-align:center;">Gagal memuat blok hadist ini.</div>'; };
                document.body.appendChild(script);
            } else {
                displayHadith();
            }
        ...
        window.backToHadithBooks = function() {
            document.getElementById('searchHadithBook').style.display = 'block';
            document.getElementById('hadithBookSelector').style.display = 'none';
            if (window.HADITH_LIST) renderHadithList(window.HADITH_LIST);
        };
                ...
                if (results.length === 0) {
                    ...
                    Object.entries(surahGroups).forEach(([name, g]) => {
                        ...
                        g.ayat.forEach(m => {
                            ...
                            searchResultHTML += `<div class="verse-group glass" style="margin-bottom:20px; padding:20px; border-radius:12px; cursor:pointer; transition:0.3s;" onclick="showTafsirs(this, ${m.surah.number}, ${m.numberInSurah}, '${m.surah.englishName.replace(/'/g, "\\'")}')" onmouseover="this.style.transform='translateY(-2px)'" onmouseout="this.style.transform='translateY(0)'">
                                <div class="ayat-header" style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px;">
                                    ...
                                    <div style="display:flex; gap:10px; flex-wrap:wrap;">
                                        <button class="btn-outline" style="border-color:var(--accent-gold); color:var(--accent-gold);" onclick="showTafsirs(this, ${m.surah.number}, ${m.numberInSurah}, '${m.surah.englishName.replace(/'/g, "\\'")}')">📖 Tafsir & Hadist</button>
                                    </div>
                                </div>
