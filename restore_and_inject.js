const fs = require('fs');
let backupHtml = fs.readFileSync('build_ui_premium_v3_backup_level4.js', 'utf8');

// 1. Google Translate CSS
backupHtml = backupHtml.replace('overflow: hidden;', `overflow: hidden;
            top: 0 !important; 
...
// 2. Google Translate JS SDK
backupHtml = backupHtml.replace('rel="stylesheet">', `rel="stylesheet">
...
// 3. Selectors
backupHtml = backupHtml.replace('<select><option>🇺🇸 English (US)</option><option>🇮🇩 Indonesia</option></select>', '<select class="proLangSelector" onchange="doGTranslate(this.value)"></select>');
backupHtml = backupHtml.replace('<select><option>🇮🇩 Indonesia</option></select>', '<select class="proLangSelector" onchange="doGTranslate(this.value)"></select>');

// 4. Hidden element
backupHtml = backupHtml.replace('<body>', '<body>\\n    <div id="google_translate_element" style="display:none;"></div>');

// 5. Arabic notranslate (USING BACKTICKS TO AVOID ESCAPE HELL)
backupHtml = backupHtml.replace(/<div style=\"font-family:'Amiri'/g, `<div class="notranslate" style="font-family:'Amiri'`);
backupHtml = backupHtml.replace(/<div style=\"font-family:\\\\'Amiri\\\\'/g, `<div class="notranslate" style="font-family:\\\\'Amiri\\\\'`);
...
backupHtml = backupHtml.replace('window.onload = initApp;', jsCode + '\\n        window.onload = initApp;');
backupHtml = backupHtml.replace('async function initApp() {', 'async function initApp() {\\n            if(typeof buildLanguageSelectors === \\'function\\') buildLanguageSelectors();');

fs.writeFileSync('build_ui_premium_v3.js', backupHtml);
console.log('Restoration and 100 Languages injection complete!');
