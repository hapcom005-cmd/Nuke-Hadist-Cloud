...
<body>
    ...
    <script>
        ...
        function openTafsirModal() {
            ...
            // Load meta if not loaded (Dari Cloud JSDelivr)
        ...
        async function loadGlobalTafsir(el, slug) {
            ...
            var style = isArabic ? "font-family:var(--font-arabic, 'Amiri'),serif; font-size:22px; line-height:2.2; direction:rtl; text-align:right;" : "font-size:15px; line-height:1.9;";
        ...
        async function showTafsir(surahNo, ayatNo) {
            ...
            try {
                if (activeGlobalTafsirSlug) {
                    window.TAFSIR_GLOBAL = window.TAFSIR_GLOBAL || {};
                    window.TAFSIR_GLOBAL[activeGlobalTafsirSlug] = window.TAFSIR_GLOBAL[activeGlobalTafsirSlug] || {};
                    ...
                    var style = isArabic ? "font-family:var(--font-arabic, 'Amiri'),serif; font-size:22px; line-height:2.2; direction:rtl; text-align:right;" : "line-height:1.8;";
        ...
        function renderHadithPage(page) {
            ...
            for(let i=start; i<end; i++) {
                ...
                <div id="hadist-\${h.number}" style="background:var(--bg-panel); border:1px solid var(--border-color); border-radius:12px; padding:30px; margin-bottom:20px; position:relative;">
                    ...
                    <div class="notranslate" style="font-family:var(--font-arabic, 'Amiri'); font-size:30px; line-height:2; text-align:right; margin-bottom:20px; color:var(--accent-gold);">\${h.arab}</div>
                    ...
                    keys.sort((a,b) => parseInt(a.split(':')[1]) - parseInt(b.split(':')[1])).forEach(k => {
                        ...
                        mapA[k].forEach((m) => {
                            html += \`<div style="background:rgba(255,255,255,0.02); padding:20px; border-radius:15px; border:1px solid rgba(245,208,97,0.2); position:relative; overflow:hidden;">
                                ...
                                <div class="notranslate" style="font-family:var(--font-arabic, 'Amiri'); font-size:26px; line-height:2; direction:rtl; text-align:right; margin-bottom:15px; color:#fff;">\${m.ar || ''}</div>
        ...
        async function searchHadist(raw) {
            ...
            for(let i=0; i<limit; i++) {
                ...
                <div style="background:var(--bg-panel); border:1px solid var(--border-color); border-radius:12px; padding:30px; margin-bottom:20px; position:relative;">
                    ...
                    <div class="notranslate" style="font-family:var(--font-arabic, 'Amiri'); font-size:30px; line-height:2; text-align:right; margin-bottom:20px; color:var(--accent-gold); margin-top:10px;">\${m.arab}</div>
        ...
        async function searchQuran(raw) {
                    ...
                    // Interceptor Kamus Satelit (Mengubah bahasa gaul ke bahasa baku API)
            let searchKw = kw;
            ...
            const isArabic = /[\u0600-\u06FF]/.test(searchKw);
                    ...
                    let kws = [kw, searchKw]; // Array for highlight
                    ...
                    try {
                        ...
                        let res = await fetch(`https://api.alquran.cloud/v1/search/${encodeURIComponent(searchKw)}/all/${edition}`);
