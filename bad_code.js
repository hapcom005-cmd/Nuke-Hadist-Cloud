...
window.renderHadistPendukung = async function(surahNum, ayatNum, containerId) {
    ...
    if (!window.HADITH_MAPPING) {
        try {
            window.HADITH_MAPPING = await fetchWithCache('QuranHadist/data/mapping_hadist_semantik_12core.json');
        } catch(e) {
            container.innerHTML = '<div style="color:red; text-align:center; padding:20px;">Gagal memuat mapping hadist dari database offline.</div>';
            return;
        }
    }
    ...
    if (!mapped || mapped.length === 0) {
        container.innerHTML = '<div style="color:var(--text-muted); text-align:center; padding:20px;">Tidak ditemukan hadist yang secara eksplisit dipetakan untuk ayat ini.</div>';
        return;
    }
    
    container.innerHTML = `<div style="color:var(--cyan); text-align:center; padding:20px; font-weight:bold;">Memuat ${mapped.length} Hadist Relevan... <span style="font-size:1.5em; display:inline-block; animation:spin 1s linear infinite;">⏳</span></div>`;
    
    let html = '';
    for (let h of mapped) {
        try {
            const data = await getHadithByNumber(h.kitab, h.id);
