...
window.switchTafsirTab = function(btn, tabName, containerId) {
    ...
    if (!container) return;
    const btns = container.querySelectorAll('.tafsir-tab-btn');
    btns.forEach(b => {
        b.classList.remove('active');
        b.style.background = 'transparent';
        b.style.border = '1px solid transparent';
        if (b.innerText.includes('Kemenag')) b.style.color = 'var(--text-muted)';
        else if (b.innerText.includes('Hadist')) b.style.color = 'var(--accent-cyan)';
        else b.style.color = 'var(--text-muted)';
    });
    ...
    const panes = container.querySelectorAll('.tafsir-content-pane');
...
window.renderHadistPendukung = async function(surah, ayat, containerId) {
    const container = document.getElementById(containerId);
    if (!container) return;
    
    container.innerHTML = "<p style='color:var(--cyan); text-align:center;'>Mencari Hadist terkait... ⏳</p>";
    
    let hadistMapping = window.HADIST_MAPPING;
    if (!hadistMapping) {
        await new Promise((resolve) => {
            if (!document.getElementById('hadist-mapping-script')) {
                const s = document.createElement('script');
                s.id = 'hadist-mapping-script';
                s.src = 'QuranHadist/data/hadist_mapping.js';
                s.onload = resolve;
                s.onerror = resolve;
        ...
        hadistMapping = window.HADIST_MAPPING;
    }
    
    if (!hadistMapping) {
        container.innerHTML = "<p style='color:red;'>Gagal memuat index hadist.</p>";
        return;
    }

    const key = surah + ":" + ayat;
    const mapped = hadistMapping[key];
    if (!mapped || mapped.length === 0) {
        container.innerHTML = "<p style='color:var(--text-muted); text-align:center;'><i>Tidak ada riwayat hadist spesifik yang ditemukan untuk ayat ini.</i></p>";
        return;
    }
    
    let hHtml = \`<h4 style="color:var(--accent-cyan); border-bottom:1px solid rgba(6,182,212,0.2); padding-bottom:10px;">Ditemukan \${mapped.length} Riwayat Hadist Terkait Ayat Ini</h4>\`;
    ...
    for (const h of mapped) {
        ...
        if (hadithObj) {
            hHtml += \`<div class="linked-hadist" style="margin:15px 0; background:rgba(255,255,255,0.05); padding:15px; border-radius:10px; border-left:3px solid var(--accent-cyan);">
    ...
    container.innerHTML = hHtml === '' ? "<p style='color:red;'>Gagal mengekstrak teks.</p>" : hHtml;
};

window.findHadistForAyat = async function(surahName, ayatNum, btn, arabText, surahNum) {
    ...
    container.style = "margin-top:15px; background:rgba(6, 182, 212, 0.05); border:1px solid rgba(6, 182, 212, 0.3); border-radius:12px; padding:20px; animation:fadeIn 0.3s ease; width:100%;";

    btn.parentElement.parentElement.parentElement.appendChild(container);

    container.innerHTML = \`
        <h4 style="color:var(--accent-cyan); margin-top:0; border-bottom:1px solid rgba(6,182,212,0.2); padding-bottom:10px; display:flex; justify-content:space-between; align-items:center;">
            <span>📚 Pencarian Hadist Khusus — Surah \${surahName} Ayat \${ayatNum}</span>
        ...
        <div id="pane_hadist_\${containerId}" style="max-height:400px; overflow-y:auto; padding-right:10px;"></div>
    \`;
    
    window.renderHadistPendukung(surahNum, ayatNum, 'pane_hadist_' + containerId);
};
...
window.showTafsirs = async function(btn, surahNum, ayatNum, surahName) {
    ...
    container.id = containerId;
    container.className = 'tafsir-kemenag-container';
    container.style = "margin-top:15px; background:rgba(212,175,55,0.05); border:1px solid rgba(212,175,55,0.3); border-radius:12px; padding:20px; animation:fadeIn 0.3s ease; width:100%;";
...
console.log("Created index_premium.html");
