...
<body>
    ...
    <script>
        ...
        function renderAyat(ayatList, surahNo) {
            ...
            ayatList.forEach(a => {
                html += \`
                <div id="ayat-\${a.nomorAyat}" style="background:var(--bg-panel); border:1px solid var(--border-color); border-radius:12px; padding:30px; margin-bottom:20px; position:relative; cursor:pointer;" onclick="showTafsir(\${surahNo}, \${a.nomorAyat})">
        ...
        function changeReciter() {
            ...
            // Stop current audio if playing, reset state
            if(currentAudio) {
                currentAudio.pause();
                currentAudio = null;
            }
            ...
            document.getElementById('audioTime').innerText = "00:00";
            
            console.log("Reciter changed to:", reciter);
            // In a real app, you would load the specific reciter's audio file here.
        }
        
        function toggleAudio() {
            const btnIcon = document.getElementById('iconPlayMain');
            const timeTxt = document.getElementById('audioTime');
            
            if(!isPlaying) {
                isPlaying = true;
                btnIcon.innerHTML = '<path d="M6 19h4V5H6v14zm8-14v14h4V5h-4z"/>'; // Pause icon
                timeTxt.innerText = "00:01 / 03:45"; // Mock time for UI demonstration
                
                // Alert for offline/online mock
                const reciter = document.getElementById('selectReciter').value;
                if(reciter.startsWith('offline_')) {
                    console.log('Memutar murottal offline...');
                } else {
                    console.log('Streaming murottal online dari API...');
                }
            } else {
                isPlaying = false;
                btnIcon.innerHTML = '<path d="M8 5v14l11-7z"/>'; // Play icon
                timeTxt.innerText = "00:00";
            }
        }
