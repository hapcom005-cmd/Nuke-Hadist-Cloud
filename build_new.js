...
html = html.replace(
    /onclick="event.stopPropagation\(\); window.fetchAudioAndPlay\('\$\{localAyatAudio\}', document.getElementById\('globalAudioPlayer'\)\)">▶ Putar<\/button>/g,
    () => `onclick="event.stopPropagation(); window.fetchAudioAndPlay('\${localAyatAudio}', document.getElementById('globalAudioPlayer'))">▶ Putar</button>
                    <button class="play-btn" style="border-color:var(--accent-gold); color:var(--accent-gold);" onclick="event.stopPropagation(); showTafsirs(this, \${s.nomor}, \${a.nomorAyat}, \`\${s.namaLatin.replace(/'/g, ' ')}\`)">📖 Tafsir & Hadist</button>`
);

html = html.replace(
    /onclick="showTafsirs\(this, \$\{m\.surah\.number\}, \$\{m\.numberInSurah\}, `\$\{m\.surah\.englishName\.replace\(\/'\/g, ' '\)\}`\)"/g,
    () => `onclick="showTafsirs(this, \${m.surah.number}, \${m.numberInSurah}, \\\`\${m.surah.englishName.replace(/'/g, ' ')}\\\`)"`
);
