...
window.showTafsirs = async function(btn, surahNum, ayatNum, surahName) {
    // KITA TARGETKAN KE PANEL KANAN (tafsir-body)
    const tafsirBody = document.getElementById('tafsir-body');
    if (!tafsirBody) return;
    
    // Tampilkan panel kanan di mode mobile jika disembunyikan
    const colInfo = document.getElementById('col-info');
    if(colInfo && window.innerWidth <= 1024) {
        colInfo.classList.add('active');
        document.getElementById('mobileOverlay').classList.add('active');
    }

    tafsirBody.innerHTML = "<p style='color:var(--cyan); text-align:center;'>Menyedot Tafsir & Hadist Offline... ⏳</p>";
    ...
    const renderData = (data) => {
        ...
        const containerId = 'tafsir_panel_container';

        tafsirBody.innerHTML = `
            <div id="${containerId}">
                <h3 style="color:var(--accent-gold); margin-top:0; border-bottom:1px solid rgba(212,175,55,0.2); padding-bottom:10px; margin-bottom:15px;">
                    📚 Surah ${surahName} : Ayat ${ayatNum}
                </h3>
                
                <div style="display:flex; flex-wrap:wrap; gap:8px; margin-bottom:15px; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:10px;">
                    <button class="tafsir-tab-btn active" onclick="switchTafsirTab(this, 'pane_kemenag', '${containerId}')" style="background:rgba(212,175,55,0.15); color:var(--accent-gold); border:1px solid var(--accent-gold); padding:6px 12px; border-radius:6px; cursor:pointer; font-weight:bold; flex-grow:1;">Kemenag RI</button>
                    <button class="tafsir-tab-btn" onclick="switchTafsirTab(this, 'pane_katsir', '${containerId}')" style="background:transparent; color:var(--text-muted); border:1px solid transparent; padding:6px 12px; border-radius:6px; cursor:pointer; font-weight:bold; flex-grow:1;">Ibnu Katsir</button>
                    <button class="tafsir-tab-btn" onclick="switchTafsirTab(this, 'pane_jalalain', '${containerId}')" style="background:transparent; color:var(--text-muted); border:1px solid transparent; padding:6px 12px; border-radius:6px; cursor:pointer; font-weight:bold; flex-grow:1;">Jalalain</button>
                    <button class="tafsir-tab-btn" onclick="switchTafsirTab(this, 'pane_quraish', '${containerId}')" style="background:transparent; color:var(--text-muted); border:1px solid transparent; padding:6px 12px; border-radius:6px; cursor:pointer; font-weight:bold; flex-grow:1;">Al-Mishbah</button>
                    <button class="tafsir-tab-btn" onclick="switchTafsirTab(this, 'pane_hadist', '${containerId}'); renderHadistPendukung(${surahNum}, ${ayatNum}, 'pane_hadist_${containerId}');" style="background:rgba(6, 182, 212, 0.1); color:var(--accent-cyan); border:1px solid var(--accent-cyan); padding:6px 12px; border-radius:6px; cursor:pointer; font-weight:bold; width:100%; margin-top:5px;">📜 Hadist Terkait</button>
                </div>

                <div id="pane_kemenag" class="tafsir-content-pane" style="overflow-y:auto; padding-right:5px; color:#fff; line-height:1.7; font-size:1.05em; text-align:justify;">
                    ${txtKemenag}
                </div>
                
                <div id="pane_katsir" class="tafsir-content-pane" style="display:none; overflow-y:auto; padding-right:5px; color:#fff; line-height:1.7; font-size:1.05em; text-align:justify;">
                    ${txtKatsir}
                </div>
                
                <div id="pane_jalalain" class="tafsir-content-pane" style="display:none; overflow-y:auto; padding-right:5px; color:#fff; line-height:1.7; font-size:1.05em; text-align:justify;">
                    ${txtJalalain}
                </div>
                
                <div id="pane_quraish" class="tafsir-content-pane" style="display:none; overflow-y:auto; padding-right:5px; color:#fff; line-height:1.7; font-size:1.05em; text-align:justify;">
                    ${txtQuraish}
                </div>

                <div id="pane_hadist" class="tafsir-content-pane" style="display:none; overflow-y:auto; padding-right:5px; color:#fff; line-height:1.7; font-size:1.05em; text-align:justify;">
