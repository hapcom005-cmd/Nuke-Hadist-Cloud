...
const new_search_logic = `async function searchQuran(raw) {
...
for (let file_path of files_to_patch) {
    if (fs.existsSync(file_path)) {
        let content = fs.readFileSync(file_path, 'utf8');
        
        let start_idx = content.indexOf("async function searchQuran(raw) {");
        ...
        if (end_idx !== -1) {
            let spacesMatch = content.substring(0, start_idx).match(/\n([\s]+)$/);
            let spaces = spacesMatch ? spacesMatch[1] : '        ';
            
            // Indent the new code
            let new_block_indented = new_search_logic.split('\n').map((line, idx) => idx === 0 ? line : spaces + line).join('\n');
            
            content = content.substring(0, start_idx) + new_block_indented + content.substring(end_idx);
